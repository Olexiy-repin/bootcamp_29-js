'use strict';
import heroesDb from './heroes.json';
import heroesTemplate from '../templates/heroes.hbs';

/*
<li class="heroes__item">
    <img src="${url}" alt="${name}" class="heroes__picture">
    <div class="heroes__content">
        <h2 class="heroes__name">${name}</h2>
        <ul class="heroes__biography list">
            <li class="heroes__biography-item"><p class="heroes__gender"><strong>Gender: </strong> ${gender}</p></li>
            <li class="heroes__biography-item"><p class="heroes__alter-egos"><strong>Alter Egos: </strong> ${alterEgos}</p></li>
        </ul>
    </div>
</li>
*/

const galleryEl = document.querySelector('.heroes-list');

// const makeTemplate = ({
//   name,
//   images: { lg: url },
//   appearance: { gender },
//   biography: { alterEgos },
// }) => {
//   if (!alterEgos) {
//     alterEgos = 'Нет альтерэго!';
//   }
//   return `
//     <li class="heroes__item">
//         <img src="${url}" alt="${name}" class="heroes__picture">
//         <div class="heroes__content">
//             <h2 class="heroes__name">${name}</h2>
//             <ul class="heroes__biography list">
//               <li class="heroes__biography-item"><p class="heroes__gender"><strong>Gender: </strong> ${gender}</p></li>
//               <li class="heroes__biography-item"><p class="heroes__alter-egos"><strong>Alter Egos: </strong> ${alterEgos}</p></li>
//             </ul>
//         </div>
//     </li>
//     `;
// };

const createTemplate = heroes => {
  // return heroes.map(makeTemplate).join(' ');
  return heroesTemplate(heroes);
};

galleryEl.insertAdjacentHTML('beforeend', createTemplate(heroesDb));
